...
if (( "$GIT_STATUS" > 0 )); then
	git pull
	cd eFormAPI/Plugins/InsightDashboard.Pn/InsightDashboard.Pn

	dotnet add InsightDashboard.Pn.csproj package Microting.eForm
	dotnet add InsightDashboard.Pn.csproj package Microting.eFormApi.BasePn
	dotnet add InsightDashboard.Pn.csproj package Microting.InsightDashboardBase
	...
	EFORM_PLUGINBASE_VERSION=`dotnet list package | grep 'Microting.InsightDashboardBase' | cut -c64-71`
	COMMIT_MESSAGE="Updating"$'\n'"- Microting.eForm to ${EFORM_VERSION}"$'\n'"- Microting.eFormApi.BasePn to ${EFORM_BASEPN_VERSION}"$'\n'"- Microting.InsightDashboardBase to ${EFORM_PLUGINBASE_VERSION}"
