...
class InsightDashboardDashboardsPage {
  ...
  deleteDashboard(rowNum: number) {
    const idx = rowNum - 1;
    cy.get('.dashboard-actions').eq(idx).find('#actionMenu').should('be.visible').click({ force: true });
    cy.wait(500);
  ...
  clearTable() {
    cy.get('.mat-mdc-row').then(($rows) => {
      ...
      for (let i = 0; i < rowCount; i++) {
        ...
        cy.get('.dashboard-actions').eq(0).find('#actionMenu').should('be.visible').click({ force: true });
        cy.wait(500);
