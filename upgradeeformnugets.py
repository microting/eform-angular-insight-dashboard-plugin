...
def main():
    ...
    if int(get_git_status()) > 0:
        ...
        os.chdir(os.path.join("eFormAPI", "Plugins", "InsightDashboard.Pn", "InsightDashboard.Pn"))
        packages = [
            'Microting.eForm', 'Microting.eFormApi.BasePn', 'Microting.InsightDashboardBase'
        ]
        process_repository('InsightDashboard.Pn.csproj', packages, 'eform-angular-insight-dashboard-plugin')

        os.chdir("..")
        os.chdir("InsightDashboard.Pn.Test")
        packages = [
            'Microsoft.NET.Test.Sdk', 'NSubstitute', 'NUnit', 'NUnit3TestAdapter',
            'NUnit.Analyzers', 'Testcontainers', 'Testcontainers.Mariadb'
        ]
        process_repository('InsightDashboard.Pn.Test.csproj', packages, 'eform-angular-insight-dashboard-plugin')
        ...
        if new_number_of_commits > current_number_of_commits:
            ...
            run_command(f"github_changelog_generator -u microting -p eform-angular-insight-dashboard-plugin -t {os.getenv('CHANGELOG_GITHUB_TOKEN')}", shell=True)
